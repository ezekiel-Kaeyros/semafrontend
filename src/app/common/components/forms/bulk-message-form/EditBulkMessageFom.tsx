...
import { EditBulkMessageTemplate, postBulkMessageTemplate } from './action';
...
const EditBulkMessageForm: React.FC<{
  modalHandler?: any;
  footer: string;
  img: string;
  body: string;
  ...
  const [valueText, setValueText] = useState<string>(props.body);
  ...
  useEffect(() => {
    ...
    setValue('footer_text', props.footer);
    // getFileHandler('');
  }, []);
  useEffect(() => {
    if (
  ...
  const onSubmit: SubmitHandler<ActivityFormValues> = async (data) => {
    ...
    try {
      if (acceptedFiles && acceptedFiles.length > 0) {
        ...
        const response = await EditBulkMessageTemplate(payload, props.id);
        if (typeof response == 'number' && response != 200) {
          toast.error('edition échouée');
          setIsLoad(false);
        } else if (typeof response == 'number' && response == 200) {
        ...
        const sessionImportKey = await getFileHandler('');
  ...
  return (
    ...
    >
      ...
      <div className="" ref={editorRef}>
        <h1 className="text-[25px] font-bold">Edit Template {props.name}</h1>
      </div>
          ...
          <div className="my-2">
            ...
            {errors.footer_text && (
              <p className="text-sm text-[red]">
                la signature ne doit pas dépasser 60 caractères
              </p>
            )}
          </div>
        </div>
      ...
      <Button
        type="submit"
        variant={isValid && btnState && !isLoad ? 'mainColor' : 'disabled'}
        iconSize={30}
        // onClick={() => //// }
        className="w-[15%] mt-8"
        disabled={isValid && btnState && !isLoad ? false : true}
      >
        {isLoad ? 'loading.....' : 'Edit'}
      </Button>
    </form>
